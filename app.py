...
def get_stock_data():
    ...
    try:
        ...
        j = resp.json()
        raw = j.get("data", [])
        # FinMind 對短時間內大量錯誤請求的 IP 會暫時封鎖（回傳 403 + msg），
        # 此時 data 欄位會是空的，讓使用者知道真正原因而非誤判成「查無股票」
        if not raw and j.get("status") == 403:
            return jsonify({"status": 502, "msg": f"FinMind API 暫時限制存取（{j.get('msg', '')}），請等待約 30 分鐘後再試"}), 502
    except Exception as e:
        return jsonify({"status": 500, "msg": f"取得股價失敗: {e}"}), 500
...
def get_sector_flow():
    ...
    try:
        ...
        return jsonify({"status": 500, "msg": f"取得法人買賣超失敗: {e}"}), 500
